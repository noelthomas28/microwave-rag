...
# Run the app locally (original, currently deployed UI)
streamlit run app/streamlit_app.py

# Run the redesigned UI (in review, not yet deployed — see UI versions below)
...
- **`app/streamlit_app.py`** — the original Streamlit UI (currently deployed), imports functions
  from `rag_engine.py`, has its own inline CSS.
- **`app/streamlit_app_v2.py`** / **`app/theme.py`** — a redesigned UI built as a separate,
  self-contained entry point rather than editing `streamlit_app.py` in place, so the deployed app
  is untouched while the redesign is reviewed. See "UI versions" below.
...
- Because Streamlit's built-in auto-scroll only fires for direct `st.chat_input` submissions, the
  ...
  of `streamlit_app.py`) — scroll-to-top on a fresh session, scroll-to-just-above-the-answer after
  a question is answered, canceling itself if the user manually scrolls.
...
There are currently two parallel Streamlit UIs sharing the same backend (`rag_engine.py`) and the
same on-disk index cache, so both behave identically functionally — only presentation differs:

- **`app/streamlit_app.py`** — the original, currently deployed UI (linked from
  `.devcontainer/devcontainer.json`'s `postAttachCommand` and used for the Streamlit Community
  Cloud deployment). Styles itself with an inline `st.markdown(f"""<style>...""")` block computed
  from `_is_dark`-branched Python variables.
- **`app/streamlit_app_v2.py`** — an Apple-inspired redesign (bigger typography, glass-card
  surfaces, staggered fade-in animations, a two-column example-question grid) built as a fully
  separate file rather than as edits to `streamlit_app.py`, so the deployed app isn't touched
  while the redesign is reviewed. It imports the same `rag_engine.py` functions and preserves
  every feature of the original (PIN gate, sidebar tips/toggles, cooking-mode search, voice
  input, source excerpts, follow-up/detail buttons, scroll-management JS, conversation export).
  Its styling comes from two places: `.streamlit/config.toml` (native widget colors, the Inter
  font, button/corner radii — via `[theme.light]`/`[theme.dark]` blocks) and `app/theme.py`'s
  `inject_theme(is_dark)` (everything `config.toml` can't reach: the hero, glass cards, pills,
  chat bubbles, animations), called once near the top of `streamlit_app_v2.py`.
  **Note:** `.streamlit/config.toml` applies to any Streamlit app run from this repo, so it will
  also subtly restyle `streamlit_app.py` (native widget colors/font/radius only — none of
  `theme.py`'s custom CSS, since that's only invoked from `streamlit_app_v2.py`).
  `streamlit_app_v2.py` is **not** wired up as the deployed entry point yet — that only happens
  once it's promoted (update `.devcontainer/devcontainer.json`'s `postAttachCommand` and whatever
  command Streamlit Community Cloud is configured to run).
