...
from theme_v3 import inject_theme_v2
from effects_v3 import inject_cursor_effects, inject_scroll_reveal
...
# .streamlit/config.toml, everything else from theme_v3.inject_theme_v2().
_theme_type = getattr(st.context.theme, "type", None) or "light"
_is_dark = _theme_type == "dark"
inject_theme_v2(_is_dark)

# Cursor-glow (desktop) / touch-ripple+ambient-drift (mobile) in the hero,
# and the scroll-reveal for the editorial sections below it. Both are
# idempotent (guarded against re-attaching on every rerun) and no-ops if
# their target elements aren't in the DOM yet/anymore, so it's safe to call
# them once here, before those elements are even rendered below.
...
# -----------------------------
# Full-bleed hero / landing moment
# -----------------------------
st.markdown(
    """
    <div class="hero-v2">
        <span class="hero-kicker">🍽️ Your LG microwave, explained</span>
        <h1><span class="hero-emoji">🤖</span>Ask Your Microwave</h1>
        <p class="hero-subtitle">Ask questions in plain English about cooking functions, settings, accessories, cleaning, and safety — answered straight from your manuals.</p>
...
# -----------------------------
# Welcome / editorial sections when chat is empty
# -----------------------------
if not st.session_state.messages:
    ...
    capabilities = [
        ("🔥", "Smoking", "Learn how to use the smoking function safely, and get flavor tips."),
        ("🍗", "Rotisserie", "Step-by-step setup for rotisserie cooking, plus the accessories you'll need."),
        ("🍕", "Cooking modes", "Find the right preset or manual mode for whatever you're making."),
        ("🍞", "Baking", "Bake cakes, breads, and more using the microwave's baking modes."),
        ("🧼", "Cleaning", "Keep your microwave running smoothly with the right cleaning routine."),
        ("⚠️", "Safety & accessories", "Know what's safe to use, and what came included with your model."),
    ]

    st.markdown('<div class="section-kicker">What I can help with</div>', unsafe_allow_html=True)
    st.markdown('<div class="section-title">Everything in your manuals, one question away</div>', unsafe_allow_html=True)
    st.markdown(
        '<div class="capabilities-grid reveal">'
        + "".join(
            f'<div class="capability-card"><span class="icon">{icon}</span>'
    ...
    st.markdown(
        '<div class="tip-card reveal">Try asking: <em>"I have chicken breast. What cooking method from the manual should I use?"</em></div>',
        unsafe_allow_html=True,
    )
...
#
# This block is ported near-verbatim from streamlit_app.py rather than
# folded into effects_v3.py: it's tightly coupled to st.session_state/rerun
# timing and, unlike the cursor-glow/scroll-reveal JS, is *meant* to
# re-attach and re-run fresh on every single rerun — a different lifecycle
# need from the "attach exactly once" effects in effects_v3.py.
if not st.session_state.messages:
    components.html(
        """
        <script>
